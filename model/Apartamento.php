...
class Apartamento {
    ...
    public function option() {
        $this->option = '';
        if ($this->option == '') {
            $this->conexao->cache->save('optionapartamento', $this->option);
            $this->option = $this->conexao->cache->read('optionapartamento');
            if ($this->option == NULL || $this->option == '') {
                $colunas = 'DISTINCT pes.apartamento, pes.bloco';
                $res = $this->procurar(NULL, $colunas);
                $qtd = $this->conexao->qtdResultado($res);
                if ($qtd > 0) {
                    while ($apartamentop = $this->conexao->resultadoArray($res)) {
                        $this->option .= '<option class="bloco' . $apartamentop['bloco'] . '" value="' . $apartamentop["apartamento"] . '">' . $apartamentop["apartamento"] . '</option>';
                    }
                } else {
...
//    }
    ...
    function procurar($post, $colunas = 'pes.*', $ca = 0) {
        ...
        $sql = 'SELECT distinct ' . $colunas . ' FROM pessoa AS pes WHERE pes.codempresa = ' . $_SESSION['codempresa'] . ' AND pes.apartamento <> "" '  . $and;

...
class Categoria {
    ...
    public function option() {
        if ($this->option == '') {
            $this->option = $this->conexao->cache->read('optioncategoria');
            if ($this->option == NULL || $this->option == '') {
                $colunas = 'DISTINCT cp.nome, cp.codcategoria';
                $res = $this->procurar(NULL, $colunas);
                $qtd = $this->conexao->qtdResultado($res);
                if ($qtd > 0) {
                    while ($categoriap = $this->conexao->resultadoArray($res)) {
                        $this->option .= '<option value="' . $categoriap["codcategoria"] . '">' . $categoriap["nome"] . '</option>';
                    }
                } else {
                    $this->option .= '<option value="">--Nada encontrado--</option>';
                }
                $this->conexao->cache->save('optioncategoria', $this->option);
            }
        }
        return $this->option;
    }
    ...
    function procurar($post = NULL, $colunas = 'cp.*', $ca = 0) {
        ...
        $sql = "SELECT SQL_CACHE {$colunas} FROM categoriapessoa AS cp WHERE codempresa = {$_SESSION['codempresa']} {$and}";

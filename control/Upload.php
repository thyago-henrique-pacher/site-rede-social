<?php
    class Upload{
        ...
        private $extnpermitida = array('exe', 'bat', 'php', 'asp', 'aspx', 'html');
        ...
        public function upload($imagem = NULL){
            ...
            if(isset($this->imagem) && $this->imagem != NULL && isset($this->imagem['name']) 
                    && (($this->imagem['size'] <= $this->tamanhoMax)||($this->imagem['size'] > $this->tamanhoMax && $this->livre_tam == "true"))){
                if($this->imagem['error'] == 0){
                    
                    $separacao        = explode('.', $this->imagem['name']);
                    $extensao         = $separacao[1];
                    if(count($separacao) > 2){
                        $extensao = $separacao[count($separacao) - 1];
                    }
                    if(!in_array($extensao, $this->extnpermitida)){
                        $this->nome_final = $this->adicional_nome . date('Y-m-d').time().'.' . $extensao; 
                        $this->moveArquivo($this->imagem['tmp_name'], "../../sistema/arquivos/". $this->nome_final);
                        $this->tamanhoArquivo = $this->imagem['size'];
                    }else{
                        $this->erro = "Por favor reveja arquivo detectamos extensão entre as não permitidas: ". implode(",", $this->extnpermitida);
                    }
